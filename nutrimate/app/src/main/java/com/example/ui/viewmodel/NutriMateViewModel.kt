...
class NutriMateViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val repository = NutritionRepository(database.mealDao(), database.groceryDao(), database.profileDao())
    ...
    // Grocery State
    val groceryItems: StateFlow<List<GroceryItem>> = repository.getAllGroceryItems()
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    fun addGrocery(nameEn: String, nameTe: String, categoryEn: String, categoryTe: String, quantity: String) {
        viewModelScope.launch {
    ...
    fun toggleGroceryBought(id: Long, currentStatus: Boolean) {
        viewModelScope.launch {
            repository.toggleGroceryBought(id, !currentStatus)
        }
    }
    ...
    fun seedHealthyGroceryStaples() {
        viewModelScope.launch {
            val staples = listOf(
                GroceryItem(nameEn = "Ragi Flour (Finger Millet)", nameTe = "రాగి పిండి", categoryEn = "Grains & Millets", categoryTe = "ధాన్యాలు & మిల్లెట్స్", quantity = "1 kg"),
                GroceryItem(nameEn = "Whole Green Moong Dal", nameTe = "పచ్చి పెసలు", categoryEn = "Proteins", categoryTe = "ప్రోటీన్ ఆహారాలు", quantity = "500 g"),
                GroceryItem(nameEn = "Fresh Spinach (Palakura)", nameTe = "పాలకూర కట్ట", categoryEn = "Fresh Greens", categoryTe = "తాజా ఆకుకూరలు", quantity = "2 bunches"),
                GroceryItem(nameEn = "Country Eggs (Farm Fresh)", nameTe = "నాటు కోడిగుడ్లు", categoryEn = "Proteins", categoryTe = "ప్రోటీన్ ఆహారాలు", quantity = "12 pcs"),
                GroceryItem(nameEn = "Fresh Curd (Probiotic)", nameTe = "తాజా పెరుగు", categoryEn = "Dairy", categoryTe = "పాల ఉత్పత్తులు", quantity = "1 packet"),
                GroceryItem(nameEn = "Apples & Guava", nameTe = "ఆపిల్స్ & జామపండ్లు", categoryEn = "Fresh Fruits", categoryTe = "తాజా పండ్లు", quantity = "1 kg")
            )
            repository.addGroceryItems(staples)
        }
    }
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            val groceryCount = repository.getAllGroceryItems().first().size
            if (groceryCount == 0) {
                seedHealthyGroceryStaples()
            }
        }
    }
}
